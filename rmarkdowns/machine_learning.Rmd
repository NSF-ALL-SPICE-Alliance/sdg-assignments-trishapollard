...
```{r}
importance_df2 <- as.data.frame(rf_obesity$importance)
```

```{r}
importance_df2_top_10 <- importance_df2 %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)
```

```{r}
ggplot(importance_df2_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
...
```{r}
pdp::partial(rf_obesity, pred.var = "normalized_score_sdg7_cleanfuel", plot = TRUE)
```

```{r}
pd2 <- pdp::partial(rf_obesity, pred.var = c("normalized_score_sdg7_cleanfuel", "normalized_score_sdg3_tb"))
# Default PDP
plotPartial(pd2)
```
