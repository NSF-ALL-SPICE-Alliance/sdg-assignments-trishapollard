...
```{r, warning=FALSE, message=FALSE}
goal_2_6_plot <- ggplot(sdr_data, aes(x = goal_2_score, 
                     y = goal_6_score,
                     label = country)) +
  geom_point() +
  geom_smooth() +
...
ggplotly(goal_2_6_plot)
```

  - in 4-6 sentences, come up with a hypotheses for the positive or negative correlation between the 2 goals
      There is a relatively positive correlation between Goal 6 scores and Goal 2 scores: correlation coefficient (0.57 out of a possible 1). As Goal 6 score increases, Clean Water and Sanitation, so does Goal 2 score, Zero Hunger. This means that Hunger increases as there is less Clean Water and Sanitation. The p-value is < 2.2e-16 is statistically significant and shows that the results did not occur by chance.
      
      - are there any points that don't follow the trend (outliers)
          Yes
          
        - which country(s) are outliers?
          Oceania, Australia, Finland, Sweden, New Zealand and United Kingdom
