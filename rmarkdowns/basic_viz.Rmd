...
```{r}
ggplot(data = sdr_data, aes(x= goal_7_score, y = goal_13_score)) +
  geom_point()
```
...
It seems that majority of the countries with a higher SDG 13 score also have higher SDG 12 scores. But, countries with lower SDG 12 scores have higher SDG 7 scores.
...
```{r}
ggplot(sdr_data, aes(x = goal_11_score)) +
  geom_histogram()
```
...
It seems that the OECD region has a higher SDG score and the Sub-Saharan African region has a lower score.