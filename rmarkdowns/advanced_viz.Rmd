...
```{r}
goal_3_4_geom_point <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr, label = country)) +
  geom_point()+
  theme_minimal()+
  scale_color_viridis_d(option = "plasma")+
  labs(title = "Regions Used for the SDR",
       x = "Goal 3 Score",
       y = "Goal 4 Score",
       color = "Region")

ggplotly(goal_3_4_geom_point)
```
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("Dark2", goal_12_score)(goal_12_score), label = mytext)
```
